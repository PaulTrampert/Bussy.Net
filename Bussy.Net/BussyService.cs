...
internal class BussyService(HandlerRegistry handlerRegistry, TransportRegistry transportRegistry, ILogger<BussyService> logger) : BackgroundService
{
    private readonly List<ITransportSubscription> _subscriptions = [];

    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        foreach (var (route, handlers) in handlerRegistry.Handlers)
        {
            var handlerList = handlers.ToList();
            IEnumerable<ITransport> transports = route.Broker is not null
                ? transportRegistry.Transports.TryGetValue(route.Broker, out var t) ? [t] : []
                : transportRegistry.Transports.Values;
